...
export function ShowcaseTotemCard({
  ...
  return (
    ...
    >
      ...
      {/* ── Real Architectural Gallery Wall Picture Frame (Widescreen 16:10) ── */}
      <div
        ref={cardRef}
        onClick={() => onOpen(project.id)}
      ...
      >
        ...
        <div className="relative w-full h-full p-2.5 sm:p-3.5 bg-[#F5F4F0] shadow-[inset_0_2px_6px_rgba(0,0,0,0.25)] flex flex-col justify-between">
          ...
          {/* Inner Artwork Cutout Window with Real High-Res Photography */}
          <div className="relative w-full flex-1 overflow-hidden bg-stone-900 shadow-[inset_0_3px_8px_rgba(0,0,0,0.4),0_1px_2px_rgba(0,0,0,0.15)] border border-stone-400/30">
            ...
            {/* Website Landing Page Presentation with High-Res Photos */}
            {project.id === 'soie-et-terre' ? (
              <img
                src="/images/showcase/soie-et-terre.jpg"
                alt="Maquette SOIE & TERRE par HBG Labs"
                loading="lazy"
                className={`w-[198%] max-w-none h-auto object-cover object-left-top transition-transform ease-in-out ${
                  isHovered ? 'duration-[5000ms] -translate-y-[28%]' : 'duration-[1000ms] translate-y-0'
                }`}
              />
            ) : (
              <img
                src="/images/showcase/kayo-construction.jpg"
                alt="Maquette KAYO CONSTRUCTION par HBG Labs"
                loading="lazy"
                className={`w-[198%] max-w-none h-auto object-cover object-left-top transition-transform ease-in-out ${
                  isHovered ? 'duration-[5000ms] -translate-y-[28%]' : 'duration-[1000ms] translate-y-0'
                }`}
